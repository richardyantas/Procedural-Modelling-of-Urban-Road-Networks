...
function drawNucleus(nucleus){	
	...
	drawFillCircle(250, 250, "red",4); // Center
}
...
function aStar(source,destiny,g){
	...
	for(var i=0;i<parents.length;i++){
		...
		if( printMajorRoads )
			drawLine(nodes[k], nodes[parents[k]], "black",2);
		k = parents[k];
	}
...
function bfs(na,g){
	var q = new Queue();
	var nb = g.adj[node.id][0];
	q.enqueue(node);

	while(!q.empty()){
		var n = q.dequeue();
		//drawCircle(n.x, n.y, "yellow");
		for(var i=0;i<g.adj[n.id].length;i++){
			var nn = g.adj[n.id][i] // Neighbour Node
			if(!g.visit[n.id][nn.id]){
				q.enqueue( nn );								
				g.visit[n.id][nn.id] = true;				
			}
		}
